...
class Model(Base):
    ...
    # CRITICAL: Stock this model is trained on - REQUIRED for 1:1 mapping
    stock_symbol = Column(Text, nullable=False, index=True)
    
    # Model save format: 'joblib' for XGBoost, 'keras' for LSTM/Transformer
    model_format = Column(Text, default='joblib')
...
class Signal(Base):
    """Trading signals generated by models - matches phase3_signals.sql"""
    __tablename__ = "signals"
    __table_args__ = (
        Index('idx_signals_model', 'model_id'),
        Index('idx_signals_instrument', 'instrument_id'),
...
class TrainingDataset(Base):
    """Track uploaded CSV datasets for model training"""
    ...
    # Metadata
    interval = Column(String, nullable=True)  # e.g., '1d', '15m'
